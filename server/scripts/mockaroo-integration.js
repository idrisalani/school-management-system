// Database Schema-Compliant Mockaroo Integration - ES Module Version
...
class SchemaCompliantMockaroo {
  ...
  async seedDatabase() {
    try {
      console.log("Starting database seeding with Mockaroo data...");

      // Generate data for each table
      const tables = ["users", "classes", "assignments"];

      for (const table of tables) {
        try {
          console.log(`Processing ${table} table...`);
          const data = await this.generateSchemaCompliantData(table, 50);
          await this.insertSchemaCompliantData(table, data);
          console.log(`Completed ${table} seeding`);
        } catch (tableError) {
          console.error(`Error seeding ${table}:`, tableError.message);
...
// CLI usage with ES modules
const args = process.argv.slice(2);
const command = args[0];

if (process.argv[1].includes("mockaroo-integration.js")) {
  const generator = new SchemaCompliantMockaroo();

  switch (command) {
    case "seed":
      generator.seedDatabase();
      break;
    case "schemas":
      generator.run();
      break;
    default:
      console.log("Usage: node mockaroo-integration.js [seed|schemas]");
      break;
  }
}

export default SchemaCompliantMockaroo;
